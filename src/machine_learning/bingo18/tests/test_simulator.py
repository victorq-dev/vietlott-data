...
def test_combined_simulation_runs(trained_model, sample_df):
    ...
    result = sim.run_combined(
        sample_df,
        bet_types=["mot_so", "cong_tong", "lon_hoa_nho"],
        mode="combine",
    )
    assert result.total_bets > 0
...
def test_combined_multiple_bet_types(trained_model, sample_df):
    sim = Bingo18Simulator(model=trained_model, budget=10_000_000, bet_size=10_000)
    result = sim.run_combined(
        sample_df,
        bet_types=["cong_tong", "lon_hoa_nho", "hai_so_trung"],
        mode="combine",
    )
    assert result.total_bets > 0
    assert result.bet_type == "cong_tong+lon_hoa_nho+hai_so_trung"
